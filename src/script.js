async function loadProjectData(folder) {
    try {
        const response = await fetch(`/src/${folder}/project.json`);
        if (!response.ok) {
        ...
        try {
            const mainImage = `/src/${folder}/main.jpg`;
        ...
        let i = 1;
        while (true) {
            const detailImage = `/src/assets/${folder}/detail${i}.jpg`;
...
async function loadIndexProjects() {
    ...
    try {
        ...
        const response = await fetch('/src/layouts.json');
        if (!response.ok) {
